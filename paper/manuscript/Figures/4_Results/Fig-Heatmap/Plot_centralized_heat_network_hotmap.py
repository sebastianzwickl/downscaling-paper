...
centralized = pyam.IamDataFrame('results_centralized+decentralized_heat_generation.xlsx')
...
if _plot_heat_map:
    ...
    cbar_ax.set_xlabel("District heating in TWh", fontsize=10)
    cbar_ax.xaxis.set_label_coords(0.5, 3.1)
    cbar_ax.xaxis.set_ticks([0,3,6,9,12])
    cbar_ax.tick_params(axis='x', labelsize= 10, pad=2)
    ...
    for _var in tuple(zip(_scenarios, _axes.flat)):
        ...
        _to_plot.plot(column="value", ax=_var[1], legend=False, cmap=cmap, vmin=0, vmax=12)
        _to_plot.boundary.plot(linewidth=0.5, ax=_var[1], color="black")
        _var[1].axis('off')
        if _var[0] == "Directed Transition":
            _var[1].set_title(r'\underline{Directed Transition}',x=0.65)
        elif _var[0] == "Societal Commitment":
            _var[1].set_title(r'\underline{Societal Commitment}',x=0.65)
        elif _var[0] == "Gradual Development":
            _var[1].set_title(r'\underline{Gradual Development}',x=0.65)
        elif _var[0] == "Techno-Friendly":
            _var[1].set_title(r'\underline{Techno-Friendly}',x=0.65)
    ...
    fig.suptitle("Heat demand supplied by district heating in 2050", fontsize=14)                                  
    plt.tight_layout()
