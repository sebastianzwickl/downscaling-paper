...
% Zwei Anmerkungen sehr wichtig. 1) Abbildung zeigt nur centralized heat supply. in jeder sub-region gibt es auch einspeisende technologien wie abbildung 1 middle die nicht centralized network sind sondern on-site wärme versorgen. Ensprechend sind alle Gebiete die völlig weiß sind vollständig über on-site technologies versorgt. 2) Weisen die markierten Gebiete auch die höchste Bevölkerungsdichte auf. diese varriiert zwischen 180 (AT239) und 5000 (AT130). 
% Guten Überblick über centralized networks aber für network topology noch deutlich zu aggregiert auf der einen seite und liefert auch entsprechend sehr geringe wärmedichte (vgl. kapitel 4.5). Consequently wird in der folgenden section die network topology detailiert untersucht und diskutiert. 
